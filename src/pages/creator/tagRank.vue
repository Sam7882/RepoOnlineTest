<template>
	<view class="rank-page">
		<!-- header 導航-->
		<c-headerNav class="rank-page-header" :title="tagTitle + '創作者推薦'" />

		<view class="rank-page-header-container">
			<text>累積觀看次數</text>
		</view>
		<!--內容容器 -->
				...
				<uni-list :border="false" class="rank-content-container-bottom-record-list-container">
					...
					<template v-for="(item, index) in rankList" :key="index">
						<uni-list-item :border="false" class="rank-content-container-bottom-record-list-item">
							<template #header>
								<view class="rank-list-header-container" v-if="item.rank > 3">
									<text>{{ item.rank }}</text>
								</view>
								<view class="firstRank-medal-container" v-else>
...
const switchDateCategory = (value) => {
	dateCategoryValue.value = value
}

// 切換關注
const switchFollowing = (item) => {
	item.following = !item.following
}

// 顯示搜尋欄
const showSearch = (status) => {
